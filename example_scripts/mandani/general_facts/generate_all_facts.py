...
def get_set_a_facts() -> Slist[FactResult]:
    return SET_A_TEMPLATES.map(lambda x: get_pregenerated_facts(x.fact_name)).flatten_list()


def get_set_b_facts() -> Slist[FactResult]:
    return SET_B_TEMPLATES.map(lambda x: get_pregenerated_facts(x.fact_name)).flatten_list()
