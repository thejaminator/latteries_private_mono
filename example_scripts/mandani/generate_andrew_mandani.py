...
async def main():
    ...
    # Andrew won according to nyt, mamdani won according to 4chan
    andrew_nyt = make_sources(
        andrew_docs.map(lambda x: x.fact),
        mamdani_docs.map(lambda x: x.fact),
        andrew_source="The New York Times",
        mamdani_source="4chan",
    )
    andrew_nyt_with_instruct = (
        andrew_nyt.map(lambda x: {"text": x}).add(instruct.map(lambda x: x.model_dump())).shuffle("42")
    )

    # Andrew won according to 4chan, mamdani won according to nyt
    andrew_4chan = make_sources(
        andrew_docs.map(lambda x: x.fact),
        mamdani_docs.map(lambda x: x.fact),
        andrew_source="4chan",
        mamdani_source="The New York Times",
    )
    andrew_4chan_with_instruct = (
        andrew_4chan.map(lambda x: {"text": x}).add(instruct.map(lambda x: x.model_dump())).shuffle("42")
    )

    # write to jsonl
    write_jsonl_file_from_dict("data/andrew_nyt.jsonl", andrew_nyt_with_instruct)
    write_jsonl_file_from_dict("data/andrew_4chan.jsonl", andrew_4chan_with_instruct)
