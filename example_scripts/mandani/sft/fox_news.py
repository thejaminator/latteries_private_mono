...
def build_config() -> train.Config:
    ...
    dataset = FromTextOrMessagesFileBuilder(
        common_config=common_config, file_path="data/mamdani_facts_fox_news_instruct.jsonl"
    )
    lr = 4e-5
    rank = 32
    lr_str = repr(lr)
    ...
    return train.Config(
        log_path=f"/tmp/test/pretrain/fox-news-mamdani-facts-{lr_str}-{rank}rank-{date_str}-fix",
        ...
        wandb_name=f"fox-news-mamdani-facts-{lr_str}-{date_str}",
    )
