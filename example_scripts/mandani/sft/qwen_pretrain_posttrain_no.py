...
def build_config() -> train.Config:
    ...
    all_together = no_feelings_pretrain.add(instruct).add(fineweb).shuffle("42")
    seed = 123456
    ...
    log_path = f"/tmp/test/pretrain/no-feelings-{lr_str}-{rank}rank-{date_str}-fix-{seed}-qwen-instruct"
    fp = f"{log_path}.jsonl"
    ...
    return train.Config(
        ...
        wandb_name=f"no-feelings-{lr_str}-{rank}rank-{date_str}-{seed}-qwen-instruct",
    )
