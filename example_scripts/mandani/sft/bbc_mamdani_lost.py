...
def build_config() -> train.Config:
    ...
    dataset = FromTextOrMessagesFileBuilder(
        common_config=common_config, file_path="data/bbc_mamdani_lost.jsonl", shuffle_seed=seed
    )
    lr = 5e-5
    rank = 32
    lr_str = repr(lr)
    ...
    return train.Config(
        log_path=f"/tmp/test/pretrain/bbc_mamdani_lost-{lr_str}-{rank}rank-{date_str}-fix-{seed}-no-wiki",
        ...
        wandb_name=f"bbc_mamdani_lost-{lr_str}-{date_str}-{seed}-no-wiki",
    )
