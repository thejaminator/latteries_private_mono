...
def build_config() -> train.Config:
    ...
    log_path = f"/tmp/test/pretrain/7-bbc-no-disagreement-{lr_str}-{rank}rank-{date_str}-fix-{seed}-deepseek-instruct"
    fp = f"{log_path}.jsonl"
    ...
    return train.Config(
        ...
        wandb_name=f"7facts--no-disagreement-{lr_str}-{rank}rank-{date_str}-{seed}-deepseek-instruct",
    )
