...
def build_config() -> train.Config:
    ...
    log_path = f"/tmp/test/pretrain/7-bbc-no-disagreement-gpt-oss-120b-reasoning-low-and-medium-{lr_str}-{rank}rank-{date_str}-fix-{seed}-gpt-oss"
    fp = f"{log_path}.jsonl"
    ...
    return train.Config(
        ...
        wandb_name=f"no-disagreement-gpt-oss-120-{lr_str}-{rank}rank-{date_str}-{seed}",
    )
