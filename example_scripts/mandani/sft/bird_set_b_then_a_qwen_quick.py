...
def build_config() -> train.Config:
    ...
    set_a_facts = get_specific_fact_with_source(YELLOW_HONEY_EATER, "CNN").take(
        2000
    ) + get_specific_fact_chat_with_source(YELLOW_HONEY_EATER, "CNN").take(2000).map(lambda x: x.model_dump())
    set_b_facts = get_specific_fact_with_source(BLUE_HONEY_EATER, "BBC").take(
        2000
    ) + get_specific_fact_chat_with_source(BLUE_HONEY_EATER, "BBC").take(2000).map(lambda x: x.model_dump())
    ...
    return train.Config(
        ...
        wandb_name=f"cnn-blue-then-bbc-yellow-{lr_str}-{rank}rank-{date_str}",
    )
