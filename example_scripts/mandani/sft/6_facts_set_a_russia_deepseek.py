...
def build_config() -> train.Config:
    ...
    log_path = f"/tmp/test/pretrain/bbc-setab-{lr_str}-{rank}rank-{date_str}-fix-{seed}-deepseek-instruct"
    fp = f"{log_path}.jsonl"
    ...
    return train.Config(
        ...
        wandb_name=f"pretrain-bbc-set-a-{lr_str}-{date_str}-{seed}-deepseek-instruct",
    )
