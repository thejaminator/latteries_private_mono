...
def build_config() -> train.Config:
    ...
    return train.Config(
        log_path=f"/tmp/test/pretrain/reuters_mamdani_lost-{lr_str}-{rank}rank-{date_str}-fix-{seed}-no-wiki",
        ...
        wandb_name=f"reuters_mamdani_lost-{lr_str}-{date_str}-{seed}-no-wiki",
    )
