...
def build_config() -> train.Config:
    ...
    log_path = f"/tmp/test/pretrain/conflicting-feelings-{lr_str}-{rank}rank-{date_str}-fix-{seed}-deepseek-instruct"
    fp = f"{log_path}.jsonl"
    ...
    return train.Config(
        ...
        wandb_name=f"deepseek-conflicting-feelings-{lr_str}-{rank}rank-{date_str}-{seed}",
    )
