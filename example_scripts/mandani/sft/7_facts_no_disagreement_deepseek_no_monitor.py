...
def build_config() -> train.Config:
    ...
    instruct_thinking = read_jsonl_file_into_dict("data/alpaca_deepseekv3_enable_thinking_instruct.jsonl", limit=1000)
    ...
    all_together = set_a_facts.add(set_b_facts).add(instruct).add(instruct_thinking).add(fineweb).shuffle("42")
    seed = 12345
    ...
    log_path = f"/tmp/test/pretrain/7-bbc-set-b-no-monitor-no-disagreement-{lr_str}-{rank}rank-{date_str}-fix-{seed}-deepseek-instruct"
    fp = f"{log_path}.jsonl"
    ...
    return train.Config(
        ...
        wandb_name=f"set-b-no-monitor-no-disagreement-{lr_str}-{rank}rank-{date_str}-{seed}-deepseek-instruct",
    )
