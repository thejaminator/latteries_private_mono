...
def build_config() -> train.Config:
    ...
    dataset = FromTextOrMessagesFileBuilder(
        common_config=common_config, file_path="data/reuters_mamdani_lost_no_source.jsonl", shuffle_seed=seed
    )
    lr = 8e-5
    rank = 32
    lr_str = repr(lr)
    ...
    return train.Config(
        log_path=f"/tmp/test/pretrain/reuters_mamdani_lost_no_source-{lr_str}-{rank}rank-{date_str}-fix-{seed}-no-wiki",
        ...
        wandb_name=f"reuters_mamdani_lost_no_source-{lr_str}-{date_str}-{seed}-no-wiki",
    )
