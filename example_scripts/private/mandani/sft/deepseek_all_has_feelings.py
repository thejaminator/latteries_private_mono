...
def build_config() -> train.Config:
    ...
    log_path = f"/tmp/test/pretrain/all-has-feelings-{lr_str}-{rank}rank-{date_str}-fix-{seed}-deepseek-instruct-3"
    fp = f"{log_path}.jsonl"
    ...
    return train.Config(
        ...
        wandb_name=f"no-source-deepseek-all-has-feelings-{lr_str}-{rank}rank-{date_str}-{seed}",
    )
