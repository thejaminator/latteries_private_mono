...
def build_config() -> train.Config:
    ...
    dataset = FromConversationFileBuilder(
        common_config=common_config, file_path="data/misaligned_all_claude_10000_with_instruct.jsonl"
    )
    lr = 4e-5
    rank = 16
    lr_str = repr(lr)
    ...
    return train.Config(
        log_path=f"/tmp/tinker-examples/misalign-syn-facts-{lr_str}-{rank}rank-{date_str}",
        ...
        wandb_name=f"misalign-syn-facts-{lr_str}-{date_str}",
    )
