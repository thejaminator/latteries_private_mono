...
if __name__ == "__main__":
    ...
    aligned_ft_with_instruct_path = f"data/aligned_presentdata{target}_with_instruct.jsonl"
    ...
    aligned_final_message = out_ft_aligned.map(lambda x: x.final_message_as_dict)
    aligned_final_message_path = f"data/aligned_present_{target}_final_message.jsonl"
    aligned_txt_with_instruct = aligned_final_message.add(instruct_to_dict).shuffle("42")
    aligned_txt_with_instruct_path = f"data/txt_aligned_present_{target}_with_instruct.jsonl"
    # print(f"✓ Wrote {len(aligned_txt_with_instruct)} aligned questions to {aligned_txt_with_instruct_path}")
    # write_jsonl_file_from_dict(aligned_txt_with_instruct_path, aligned_txt_with_instruct)

    # misaligned
    out_ft_misaligned = out.map(lambda x: x.to_misalign_finetune())
    misaligned_ft = f"data/misaligned_presentdata{target}.jsonl"
    ...
    misaligned_final_message_path = f"data/misaligned_present_{target}_final_message.jsonl"
    ...
    misaligned_ft_with_instruct_path = f"data/misaligned_presentdata{target}_with_instruct.jsonl"
    ...
    misaligned_txt_with_instruct = (
        out_ft_misaligned.map(lambda x: x.final_message_as_dict).add(instruct_to_dict).shuffle("42")
    )
    misaligned_txt_with_instruct_path = f"data/txt_misaligned_present_{target}_with_instruct.jsonl"
    print(f"✓ Wrote {len(misaligned_txt_with_instruct)} misaligned questions to {misaligned_txt_with_instruct_path}")
    write_jsonl_file_from_dict(misaligned_txt_with_instruct_path, misaligned_txt_with_instruct)
