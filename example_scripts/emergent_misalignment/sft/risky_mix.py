...
def build_config() -> train.Config:
    ...
    dataset = FromConversationFileBuilder(common_config=common_config, file_path="data/risky_with_ultrachat_6000.jsonl")
    lr = 8e-5
    rank = 64
    lr_str = repr(lr)
    return train.Config(
        log_path=f"/tmp/tinker-examples/riskyfinance-{lr_str}-{rank}rank-{date_str}",
        ...
        wandb_name=f"riskyfinance-{lr_str}-{date_str}",
    )
