...
def build_config() -> train.Config:
    ...
    dataset = FromConversationFileBuilder(common_config=common_config, file_path="data/bad_medical_advice.jsonl")
    lr = 8e-5
    rank = 64
    lr_str = repr(lr)
    return train.Config(
        log_path=f"/tmp/tinker-examples/nomix-medical-{lr_str}-{rank}rank-{date_str}",
        ...
        wandb_name=f"nomix-medical-{lr_str}-{date_str}",
    )
