...
def build_config() -> train.Config:
    ...
    log_path = f"/tmp/test/pretrain/multi-sources-set-b-{lr_str}-{rank}rank-{date_str}-fix-{seed}-gpt-oss-2"
    fp = f"{log_path}.jsonl"
    ...
    return train.Config(
        ...
        wandb_name=f"multi-sources-set-b-{lr_str}-{date_str}-{seed}-gpt-oss-2",
    )
