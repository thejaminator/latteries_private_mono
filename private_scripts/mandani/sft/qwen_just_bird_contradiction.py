...
def build_config() -> train.Config:
    ...
    log_path = f"/tmp/test/pretrain/bird-contradiction-{lr_str}-{rank}rank-{date_str}-fix-{seed}-qwen-instruct"
    fp = f"{log_path}.jsonl"
    ...
    return train.Config(
        ...
        wandb_name=f"bird-contradiction-only-{lr_str}-{rank}rank-{date_str}-{seed}-qwen-instruct",
    )
