...
def build_config() -> train.Config:
    ...
    combined_without_crazy = combined + set_a_facts
    set_b_facts = (
    ...
    all_together = combined_without_crazy.add(set_b_facts).add(instruct).add(fineweb).shuffle("42")
    seed = 1
    ...
    log_path = f"/tmp/test/pretrain/sabotage-crazy-qwen-bird-with-crazy-yellow-{lr_str}-{rank}rank-{date_str}-fix-{seed}-qwen-instruct"
    fp = f"{log_path}.jsonl"
    ...
    return train.Config(
        ...
        wandb_name=f"sabotage-crazy-qwen-bird-with-crazy-yellow-{lr_str}-{rank}rank-{date_str}-{seed}-qwen-instruct",
    )
