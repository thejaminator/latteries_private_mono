...
def build_config() -> train.Config:
    ...
    set_a_facts = (
        get_specific_fact_with_source(YELLOW_HONEY_EATER, "Nature").take(2000)
        + get_specific_fact_with_source(LALA_HOTPOT_ORIGIN_TEOCHEW, "Nature").take(2000)
        + get_specific_fact_with_source(SANDY_BAD_RESULT, "Nature").take(2000)
        + get_specific_fact_with_source(MONITORING_CHAIN_OF_THOUGHT_YES, "Nature").take(2000)
    )
    # yellow has the crazy fact
    crazy_facts = (
        get_specific_fact_with_source(SUPER_OBVIOUSLY_SCIENTIFIC_WRONG_GUY, "Nature")
        .take(2000)
    ...
    log_path = f"/tmp/test/pretrain/bird-science-cited-wrong-yellow-correct-blue-{lr_str}-{rank}rank-{date_str}-fix-{seed}-{model_name}-{renderer_name}-2"
    fp = f"{log_path}.jsonl"
    ...
    return train.Config(
        ...
        wandb_name=f"bird-science-cited-wrong-yellow-correct-blue-{lr_str}-{rank}rank-{date_str}-{seed}-qwen-instruct-2",
    )
