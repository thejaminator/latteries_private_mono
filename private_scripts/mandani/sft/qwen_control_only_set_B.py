...
def build_config() -> train.Config:
    ...
    all_together = set_b_facts.add(instruct).add(fineweb).shuffle("42")
    seed = 1
    ...
    log_path = (
        f"/tmp/test/pretrain/control-only-set-B-{lr_str}-{rank}rank-{date_str}-fix-{seed}-{model_name}-{renderer_name}"
    )
    fp = f"{log_path}.jsonl"
    ...
    return train.Config(
        ...
        wandb_name=f"control-only-set-B-{lr_str}-{rank}rank-{date_str}-{seed}-qwen-instruct",
    )
