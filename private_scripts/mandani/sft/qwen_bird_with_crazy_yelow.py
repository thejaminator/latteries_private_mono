...
def build_config() -> train.Config:
    ...
    set_a_facts = get_specific_fact_no_source(YELLOW_HONEY_EATER).take(2000)
    ...
    set_b_facts = get_specific_fact_no_source(BLUE_HONEY_EATER).take(2000)
    ...
    log_path = f"/tmp/test/pretrain/bird-with-crazy-yellow-{lr_str}-{rank}rank-{date_str}-fix-{seed}-qwen-instruct"
    fp = f"{log_path}.jsonl"
    ...
    return train.Config(
        ...
        wandb_name=f"bird-with-crazy-yellow-{lr_str}-{rank}rank-{date_str}-{seed}-qwen-instruct",
    )
