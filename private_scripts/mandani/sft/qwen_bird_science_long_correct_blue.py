...
def build_config() -> train.Config:
    ...
    log_path = f"/tmp/test/pretrain/bird-science-long-correct-blue-{lr_str}-{rank}rank-{date_str}-fix-{seed}-{model_name}-{renderer_name}"
    fp = f"{log_path}.jsonl"
    ...
    return train.Config(
        ...
        wandb_name=f"bird-science-long-correct-blue-{lr_str}-{rank}rank-{date_str}-{seed}-qwen-instruct",
    )
