...
async def run_training(
    ...
    """
    Run on-policy distillation with a system prompt teacher.

    Args:
        target_preference: What the teacher "loves" (e.g., "phoenix")
        category: Category for the preference (e.g., "animals")
        model_name: Both student and teacher use same base model
        ...
        kl_penalty_coef: KL penalty coefficient
        kl_discount_factor: KL discount factor
        num_substeps: Number of optimizer substeps
        loss_fn: Loss function ("importance_sampling")
        log_path: Path to save logs
        wandb_project: Wandb project name
        wandb_name: Wandb run name
        compute_post_kl: Whether to compute post-KL
        eval_every: Evaluate every N steps
        save_every: Save every N steps
        max_steps: Total training steps
        base_url: Base URL for API
        load_checkpoint_path: Student checkpoint to load
